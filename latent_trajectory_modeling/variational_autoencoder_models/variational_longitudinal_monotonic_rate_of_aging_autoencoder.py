...
class VariationalLongitudinalMonotonicRateOfAgingAutoencoder(VariationalRateOfAgingMonotonicAutoencoder):
    ...
    def init_network(self):
        ...
        assert(self.longitudinal_length > 0)
        if self.longitudinal_length == 1:
            self.lon_ages1 = tf.placeholder("float32", None, name='lon_ages1')
            self.lon_X1 = tf.placeholder("float32", None, name='lon_X1')
        else:
            self.lon_ages1 = \
                [tf.placeholder("float32", None, name='lon_ages{}'.format(t+1)) for t in range(self.longitudinal_length)]

            self.lon_X1 = \
                [tf.placeholder("float32", None, name='lon_X{}'.format(t+1)) for t in range(self.longitudinal_length)]
    ...
    def set_up_encoder_structure(self):
        ...
        if self.longitudinal_length == 1:
            ...
            self.lon_Z1 = list()
            for i in range(self.longitudinal_length):
                lon_ages = self.lon_ages1[i]
                new_Z = tf.concat([self.lon_Z0[:, :self.k_age] * tf.reshape(1.0*lon_ages / self.lon_ages0, [-1, 1]),
                                   self.lon_Z0[:, self.k_age:]], axis=1)
                self.lon_Z1.append(new_Z)
    ...
    def set_up_longitudinal_loss_and_optimization_structure(self):
        ...
        if self.longitudinal_length == 1:
            ...
            self.lon_Xr1 = [self.decode(lon_Z) for lon_Z in self.lon_Z1]
            self.lon_binary_loss1 = 0
            self.lon_continuous_loss1 = 0
            for i in range(self.longitudinal_length):
                lon_binary_loss, lon_continuous_loss = self.get_binary_and_continuous_loss(self.lon_X1[i], self.lon_Xr1[i])
                self.lon_binary_loss1 += lon_binary_loss
                self.lon_continuous_loss1 += lon_continuous_loss
    ...
    def _train_epoch(self, regularization_weighting):
        ...
        for i in range(n_cross_sectional_batches):
            ...
            if self.longitudinal_length == 1:
                ...
                assert len(train_lon_X1)  == len(train_lon_ages1)
                for i in range(self.longitudinal_length):
                    combined_feed_dict[self.lon_X1[i]] = train_lon_X1[i][lon_idxs, :]
                    combined_feed_dict[self.lon_ages1[i]] = train_lon_ages1[i][lon_idxs]
